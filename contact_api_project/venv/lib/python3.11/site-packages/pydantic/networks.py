...
class PostgresDsn(_BaseMultiHostUrl):
    ...
    * Supports multiple hosts
    ...
    m = MyDatabaseModel(db='postgres://user:pass@127.0.0.1:5432/foobar')
    print(m.db)
    #> postgres://user:pass@127.0.0.1:5432/foobar

    try:
        MyDatabaseModel(db='postgres://user:pass@127.0.0.1:5432')
    except ValidationError as e:
        print(e)
        '''
        ...
        assert (None)
         +  where None = PostgresDsn('postgres://user:pass@127.0.0.1:5432').path [type=assertion_error, input_value='postgres://user:pass@127.0.0.1:5432', input_type=str]
        '''
    ```
    """
...
class RedisDsn(AnyUrl):
    ...
    * User info required
    * TLD not required
    * Host required (e.g., `rediss://:pass@127.0.0.1`)
    """
...
class KafkaDsn(AnyUrl):
    ...
    * Host not required
    ...
    _constraints = UrlConstraints(allowed_schemes=['kafka'], default_host='127.0.0.1', default_port=9092)
...
class NatsDsn(_BaseMultiHostUrl):
    ...
    _constraints = UrlConstraints(
        allowed_schemes=['nats', 'tls', 'ws', 'wss'], default_host='127.0.0.1', default_port=4222
    )
